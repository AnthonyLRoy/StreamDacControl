...
SPI_t spiHost2 = spibus::SPI(SPI2_HOST);
...
namespace spibus
{
    ...
    void spi_init(gpio_num_t serial_data_out, gpio_num_t data_clock_out)
    {
        sdo = serial_data_out;
        clk = data_clock_out;
        esp_err_t ret;
        ...
        spi_bus_config_t buscfg = {
            .miso_io_num = sdo, // No MISO since it's receive-only
            .sclk_io_num = clk,
            .quadwp_io_num = -1,
            .quadhd_io_num = -1,
    ...
    void spi_send_only(uint16_t dataToSend)
    {
        esp_err_t ret;
        ...
        uint16_t tx_data = 0x55;   // Example data to send (0x55)
